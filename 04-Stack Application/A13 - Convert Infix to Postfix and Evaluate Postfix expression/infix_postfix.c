...
int Infix_Postfix_conversion(char *Infix_exp, char *Postfix_exp, Stack_t *stk)
{
    ...
    while (Infix_exp[i] != 0)
    {
        // If Infix_exp[i] is withiin range of 48 and 57 then store that char in Postfix_exp[j] and j++.
        ...
        else
        {
            ...
            else if (Infix_exp[i] == ')')
            {
                ...
                {
                    // Store the top most element of stack in Postfix_exp[j] then pop the same element and j++.
            ...
            else
            {
                ...
                while (priority(Infix_exp[i]) <= priority(stk->stack[stk->top]))
                {
                    ...
                    // store the top most element in stack in Psotfix_exp[j] and pop the top most element in stack and j++
                    Postfix_exp[j] = peek(stk);
                    pop(stk);
                    j++;
                }
    ...
    if (Infix_exp[i] == 0)
    {
        // Pop all the elemnts in stack in Postfix[j].
        while (stk->top != -1)
        {
            Postfix_exp[j] = peek(stk);
            pop(stk);
